...
function normalizeNotificationOptions(data = {}) {
  const sound = String(data.sound || "default") === "default" ? "default" : "disabled";
  const badge = String(data.badge || "1") === "1" ? 1 : null;
  const expiration = data.expiration || {};
  const expirationValue = Number.parseInt(String(expiration.value || "1"), 10);
  const expirationUnit = String(expiration.unit || "days");
  const unitSeconds = EXPIRATION_UNIT_SECONDS[expirationUnit] || EXPIRATION_UNIT_SECONDS.days;
  ...
  return {
    ...
    expirationUnit: EXPIRATION_UNIT_SECONDS[expirationUnit] ? expirationUnit : "days",
...
function getMessagingClient() {
  if (!messagingInstance) {
    messagingInstance = require("firebase-admin/messaging").getMessaging();
  }
...
async function sendWeeklyGridNotification(gridID, gridData) {
  const body = "La grille de la semaine est disponible !";
  const delivery = await sendAllUsersNotification({
    ...
    options: {
      ...
      expirationValue: 1,
      expirationUnit: "days",
      expirationSeconds: EXPIRATION_UNIT_SECONDS.days,
    },
  });
...
async function sendAllUsersNotification({ title, body, data = {}, options = {} }) {
  ...
  const results = await Promise.all(
    batches.map((batch) => getMessagingClient().sendEachForMulticast({
      tokens: batch,
